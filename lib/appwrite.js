...
export const searchFeaturedSchools = async (query) => {
    try {
...
export const likePost = async (postId, userId) => {
    try {
        ...
        const post = await databases.getDocument(databaseId, postsByUserCollectionId, postId);
        const currentLikes = post.liked_by || [];
...
export const unlikePost = async (postId, userId) => {
    try {
        ...
        const likedPost = await getPostLikedByUser(postId, userId);
        //console.log("liked post", likedPost.liked_by);
        if (likedPost && likedPost.liked_by) {
            const index = likedPost.liked_by.indexOf(userId);
            if (index > -1) {
                likedPost.liked_by.splice(index, 1);
            }
        }
...
export const likeComment = async (commentId, userId) => {
    try {
        ...
        const comment = await databases.getDocument(databaseId, commentsByUserCollectionId, commentId);
        const currentLikes = comment.liked_by || [];
...
export const unlikeComment = async (commentId, userId) => {
    try {
        ...
        const likedComment = await getCommentLikedByUser(commentId, userId);
        
        if (likedComment && likedComment.liked_by) {
            const index = likedComment.liked_by.indexOf(userId);
            if (index > -1) {
                likedComment.liked_by.splice(index, 1);
            }
        }
...
export const likeReply = async (replyId, userId) => {
    try {
        ...
        const reply = await databases.getDocument(databaseId, repliesByUserCollection, replyId);
        const currentLikes = reply.liked_by || [];
...
export const unlikeReply = async (replyId, userId) => {
    try {
        ...
        const likedReply = await getReplyLikedByUser(replyId, userId);
        
        if (likedReply && likedReply.liked_by) {
            const index = likedReply.liked_by.indexOf(userId);
            if (index > -1) {
                likedReply.liked_by.splice(index, 1);
            }
        }
